...
%[t,x]= ExplicitEulerAdaptiveStep(@CSTR3, [t0, tf], InitCond, h0, abstol, reltol, u, p);


%%% Plotting and testing for different tolerances
tspan = [0, 5];
...
for i = 1:3
    ...
    hold on;
    for tol = reltol
        [T, X] = ExplicitEulerAdaptiveStep(@CSTR3, tspan, InitCond, h0, abstol, tol, u, p);
        plot(T, X(:,i), 'DisplayName', sprintf('reltol = %d', tol));
    end

    % Reference solution using ode15s
    [Tref, Xref] = ode15s(@(t,x) CSTR3(t,x,u,p), tspan, InitCond);
    plot(Tref, Xref(:,i), 'k--', 'DisplayName', 'ode15s');
