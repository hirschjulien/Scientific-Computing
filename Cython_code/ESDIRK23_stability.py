...
# %%
real_range = np.linspace(-5.0, 2.5, 600)  # Re range
imag_range = np.linspace(-5.0, 5.0, 600)  # Im range
...
for i in range(len(real_range)):
    for j in range(len(imag_range)):
        ...
        # use ESDIRK23 solver
        t_history, x_history, t_adaptive_size, t_adaptive = ESDIRK23_solver(
...
pic1.set_title('Stability plot for ESDIRK23 Method', fontsize=20, fontweight='bold', color='#333333', y=0.98)
...
plt.tight_layout()
plt.savefig("ESDIRK23_stability.png")
plt.show()
